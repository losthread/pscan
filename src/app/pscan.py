...
# print the results
def show_results(verbose, port, state, aggressive, target):
  if verbose:
    print(f"Port {port}: {state}", end="")
...
def scan(ports: list, target, verbose=False, scan_type='tcp_connect', aggressive=False):
  ...
  if scan_type == 'tcp_connect':
    for port in ports:
      ...
      show_results(verbose, port, state, aggressive, target)
  ...
  # 2 way tcp scan (half open)
  elif scan_type == 'syn':
    for port in ports:
      # create a TCP SYN packet by giving it a flag
      packet = IP(dst=target)/TCP(dport=port,flags='S')
      # sr1 sends one packet and waits ONLY FOR 1 REPLY ( send syn, get syn + ack or rst + ack, and send rst)
      ...
      # getting a response
        ...
        # craft RST to send to end connection halfway

        RST = IP(dst=target)/TCP(dport=port, sport=response[TCP].dport, seq=response[TCP].ack, ack=response[TCP].seq + 1, flags="R")
        # flags is a bit string FIN SYN RST PSH ACK URG
        #                       0   1   0   0   1   0
  ...
  # Check firewall tcp ack scan
  elif scan_type == "ack":
    for port in ports:
      # Create TCP ACK packet
      packet = IP(dst=target)/TCP(dport=port, flags="A")

      # Send packet and wait for one reply
      response = sr1(packet, timeout=1, verbose=0)

      # No reply or ICMP = filtered
      if response is None or response.haslayer(ICMP):
        state = "FILTERED"
...
def main():
  ...
  if args.all_ports:
    ...
    scan(all_ports, args.target, verbose=args.v, scan_type=scan_type, aggressive=is_aggressive)

  elif args.port_range and '-' in args.port_range:
    start, end = args.port_range.split('-')
    ports = list(range(int(start), int(end) + 1))
    scan(ports, args.target, verbose=args.v, scan_type=scan_type, aggressive=is_aggressive)
    ...
    scan(top_100_ports, args.target, verbose=args.v, scan_type=scan_type, aggressive=is_aggressive)

  else:
    scan(top_1000_ports, args.target, verbose=args.v, scan_type=scan_type, aggressive=is_aggressive)
